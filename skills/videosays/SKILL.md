...
Use `npx videosays@1.3.0` to submit video links and retrieve transcript text or subtitles. Commands pin the CLI to the tested release. The CLI sends the configured API key and submitted links/share text to the configured Videosays API. CLI source: https://github.com/xwchris/videosays-agent-tools/tree/v1.3.0/packages/cli .
...
- Existing installations need a CLI update to receive the current endpoint and network handling: `npm install -g videosays@1.3.0`, or use `npx videosays@1.3.0`. An older CLI can call the API with `VIDEOSAYS_API_URL=https://api.videosays.cn`, but it will not have the HTTP/2-first transport, DNS address rotation, or network diagnostics.
...
```bash
npx videosays@1.3.0 whoami
```

If authentication is missing, run:

```bash
npx videosays@1.3.0 login
```

Ask the user to open the printed authorization URL, sign in, and approve the CLI. If the user explicitly provides an API key, run `npx videosays@1.3.0 login --api-key "$VIDEOSAYS_API_KEY"`. Never print or reveal the API key.
...
```bash
VIDEOSAYS_CLIENT_SURFACE=agent_skill VIDEOSAYS_CLIENT_NAME=videosays-skill npx videosays@1.3.0 transcribe "<video-link-or-share-text>"
```
...
```bash
npx videosays@1.3.0 status "<task-id>"
```
...
```bash
npx videosays@1.3.0 status "<task-id>" --format timeline
npx videosays@1.3.0 status "<task-id>" --format srt
npx videosays@1.3.0 status "<task-id>" --format vtt
```
...
```bash
VIDEOSAYS_CLIENT_SURFACE=agent_skill VIDEOSAYS_CLIENT_NAME=videosays-skill npx videosays@1.3.0 batch links.txt
```
...
```bash
npx videosays@1.3.0 batch status "<batch-id>"
```
...
```bash
npx videosays@1.3.0 batch continue "<batch-id>"
```
...
```bash
npx videosays@1.3.0 transcribe "<video-link>" --wait
npx videosays@1.3.0 batch links.txt --wait
```
...
```bash
npx videosays@1.3.0 balance
npx videosays@1.3.0 history
npx videosays@1.3.0 doctor
npx videosays@1.3.0 batch cancel "<batch-id>"
```
...
For connection errors, run `npx videosays@1.3.0 doctor` and share its output with support. The command probes the configured API's resolved addresses without sending the API key.
