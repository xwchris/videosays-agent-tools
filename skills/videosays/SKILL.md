...
Use `npx videosays@1.2.8` to submit video links and retrieve transcript text or subtitles. Commands pin the CLI to the tested release. The CLI sends the configured API key and submitted links/share text to the configured Videosays API. CLI source: https://github.com/xwchris/videosays-agent-tools/tree/v1.2.8/packages/cli .
...
- Existing installations need a CLI update to receive the new default: `npm install -g videosays@1.2.8`, or use `npx videosays@1.2.8`. An older CLI can call the API with `VIDEOSAYS_API_URL=https://api.videosays.cn`, but its login/recharge links may still use `.com` until upgraded.
...
```bash
npx videosays@1.2.8 whoami
```

If authentication is missing, run:

```bash
npx videosays@1.2.8 login
```

Ask the user to open the printed authorization URL, sign in, and approve the CLI. If the user explicitly provides an API key, run `npx videosays@1.2.8 login --api-key "$VIDEOSAYS_API_KEY"`. Never print or reveal the API key.
...
```bash
VIDEOSAYS_CLIENT_SURFACE=agent_skill VIDEOSAYS_CLIENT_NAME=videosays-skill npx videosays@1.2.8 transcribe "<video-link-or-share-text>"
```
...
```bash
npx videosays@1.2.8 status "<task-id>"
```
...
```bash
npx videosays@1.2.8 status "<task-id>" --format timeline
npx videosays@1.2.8 status "<task-id>" --format srt
npx videosays@1.2.8 status "<task-id>" --format vtt
```
...
```bash
VIDEOSAYS_CLIENT_SURFACE=agent_skill VIDEOSAYS_CLIENT_NAME=videosays-skill npx videosays@1.2.8 batch links.txt
```
...
```bash
npx videosays@1.2.8 batch status "<batch-id>"
```
...
```bash
npx videosays@1.2.8 batch continue "<batch-id>"
```
...
```bash
npx videosays@1.2.8 transcribe "<video-link>" --wait
npx videosays@1.2.8 batch links.txt --wait
```
...
```bash
npx videosays@1.2.8 balance
npx videosays@1.2.8 history
npx videosays@1.2.8 batch cancel "<batch-id>"
```
