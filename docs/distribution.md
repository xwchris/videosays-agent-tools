...
- Generic portable package: generated by `npm run build:skill:portable`; its version also comes from `distribution/skill-version.json`.
...
Output: `dist/workbuddy/videosays-workbuddy-<Skill-version>.zip`, with a root `SKILL.md`. The independently maintained Skill version is taken from `distribution/skill-version.json`; CLI releases do not change it. Increment it only when the Skill or its listing package changes. This prepares files locally and does not upload or publish them.
